document.addEventListener('DOMContentLoaded', () => {
    // Para a página de Maniçoba, os botões de "Pedir Agora" já contêm os links diretos para o WhatsApp no HTML.
    ...
    /*
    const orderButtons = document.querySelectorAll('.btn-manicoba');

    orderButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            event.preventDefault(); // Impede o redirecionamento imediato

            const productName = button.closest('.manicoba-item').querySelector('h3').textContent;
            const productPrice = button.closest('.manicoba-item').querySelector('.manicoba-price').textContent;
